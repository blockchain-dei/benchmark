...
const cityList = [
    'Sao Joao da Boa Vista', 'Coimbra', 'New York', 'Porto', 'Lisbon', 'Sao Paulo', 'Campinas', 'Buenos Aires', 'Paris', 'Belo Horizonte', 'Rio de Janeiro', 'Coritiba', 'Nazare', 'Madrid', 'Barcelona'
];
...
class ContractState {
    ...
    */
    ...
    getRandomEncryptKey()
    {
        //console.log('antes getRandomKey');
        var id = crypto.randomBytes(32).toString('hex');
        return id;
    }
    ...
    getRandomZipCode() {
        //console.log('randomZip',randomZip(true))
        return  randomZip(true);
    }
    ...
    {
        let vCity;
        vCity = cityList[Math.floor(Math.random() * cityList.length)];
        return vCity;
    }
    ...
    getRecordDetailsArg(pPatientId)
    {

        return {
            _unique_id: pPatientId,
        }
    }
    getStartVisit(vAppointmentId) {
        const currentDate = new Date();
        let vtimestamp = currentDate.getTime();

        return {
            unique_id: vAppointmentId,
            _time: vtimestamp
        }
    ...
    getBirthdDayDate() {
        const x = this.getRandomIntInterval(18, 60);
        const birthDay = new Date();
        birthDay.setDate(birthDay.getDate() - x);
        let vtimestamp = birthDay.getTime();
        //console.log('getBirthdDayDate', vtimestamp)
        return  vtimestamp;
    }
    ...
    getRandomFone()
    {
        //console.log('getRandomFone',randomPhone());
        return randomPhone();
    }
    ...
    getPatientInfoArgs() {
        const vCity =  this.getRandomCity();
        const vFirstName = this.getRamdomFirstname();
        const vLastName = this.getRamdomLastname();
        const vEncrypt = this.getRandomEncryptKey();
        //console.log('getPatientInfoArgs', vEncrypt );
  

        return {
            _firstName: vFirstName,
            _lastName: vLastName,
            _IID: this.getRandomInt(500),
            _bdate: this.getBirthdDayDate(),
            _email: this.getRandomEmail(),
            _phone: this.getRandomFone(),
            _zip: this.getRandomZipCode(),
            _city: vCity,
            _encryption_key: vEncrypt,
